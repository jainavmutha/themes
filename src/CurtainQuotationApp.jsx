...
.status-select:focus {
  ...
  .hero-actions .user-pill {
    ...
    border-radius: 999px !important;
    background: linear-gradient(135deg, #FFFFFF 0%, #FFFCF8 100%) !important;
    border: 1px solid #E9DED4 !important;
    box-shadow: 0 6px 16px rgba(46,46,46,0.045) !important;
    color: var(--primary-dark) !important;
    font-size: 12px !important;
  }

  .hero-actions .user-role {
  ...
  .hero-actions .btn:hover {
    ...
    box-shadow: 0 10px 22px rgba(46,46,46,0.08) !important;
  }

  .hero-actions .btn-outline {
    background: #FFFFFF !important;
    border: 1px solid rgba(183,7,102,0.24) !important;
    color: var(--primary) !important;
  }

  .hero-actions .btn-outline:hover {
    background: #FFF5FA !important;
    border-color: rgba(183,7,102,0.42) !important;
    color: var(--primary) !important;
  }
  ...
  .hero-actions button.btn-outline:first-of-type {
    background: #FFFFFF !important;
    border: 1px solid #EBC0D6 !important;
    color: var(--primary) !important;
  }

  .hero-actions .btn-danger:hover,
  .hero-actions button.btn-outline:first-of-type:hover {
    background: #FFF5FA !important;
  ...
  @media (min-width: 900px) {
    ...
    .hero-actions .btn:nth-last-child(1) {
...
function computeRoomCost(room, settings) {
  ...
  const fabricBreakdowns = fabrics.map((fab) => {
    ...
    const fabricTrackCost = (fc.isWallpaper || fc.isMattress || fc.blindType) ? 0 : (fc.isRomanBlind ? (fc.widthFeet || 0) * trackRate : (room.needInstallation ? fc.trackFeet * trackRate : 0));
...
/* =========================
   ...
   GST rules used here:
   1. Discount applies only to fabric/material value.
   2. Fabric/material GST is calculated on discounted material value.
   3. Stitching, lining, track, installation and miscellaneous costs are treated as services/other at 18%.
   ========================= */
function computeGstBreakdown(rooms, commercials, settings, miscellaneousCosts = []) {
  ...
  roomTotals.forEach(({ cost }) => {
    cost.fabricBreakdowns.forEach(fb => {
      let materialCategory = fb.gstCategory;

      if (!materialCategory || !materialCategory.id) {
        if (fb.isWallpaper) materialCategory = wallpaperCategory;
        else if (fb.blindType) materialCategory = blindCategory;
        else materialCategory = fabricCategory;
      }

      // Discount is applied only on fabric/material value.
      const discountedMaterialCost = Number(fb.clothCost || 0) * fabricDiscountRatio;
      addToCategory(materialCategory, discountedMaterialCost);

      // Stitching, lining and track are not discounted and are charged at 18% GST.
      const serviceBase =
        Number(fb.stitchingCost || 0) +
        Number(fb.liningCost || 0) +
        Number(fb.trackCost || 0);

      addToCategory(serviceCategory, serviceBase);
    });

    // Installation is also treated as service at 18% GST.
    addToCategory(serviceCategory, Number(cost.installationCost || 0));
  });

  (miscellaneousCosts || []).forEach(item => {
    const miscBase = toNum(item.rate) * (toNum(item.quantity) || 1);

    // Miscellaneous costs are always charged at 18% GST.
    addToCategory(serviceCategory, miscBase);
  });
...
/* =========================
   ...
   ========================= */
async function generatePerformaInvoice(rooms, meta, settings, miscellaneousCosts = []) {
  const logoDataURL   = await imageToDataURL(meta.company.logoUrl);
...
export default function CurtainQuotationApp() {
  ...
  onClick={async () => {
    try {
      ...
      const doc = await generatePerformaInvoice(rooms, meta, settings, miscellaneousCosts);
      doc.save(`PerformaInvoice_${quoteMeta.customerName || 'Customer'}_${quoteNo || 'Draft'}.pdf`);
    } catch (err) {
      console.error(err);
...
>
  ...
  onClick={async () => {
    try {
      ...
      doc.save(`PerformaInvoice_${rec.customer?.name || 'Customer'}_${rec.quoteNo}.pdf`);
    } catch (err) {
      console.error(err);
      alert('Could not generate Performa Invoice.');
    }
  }}
>
  PI
